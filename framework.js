...
const SUPPORTED_REQUEST_SOURCES = ["headers", "params", "query", "body"];

export function validateFrameworkConfig({ rule, options } = {}) {
  if (!rule || typeof rule !== "object" || Array.isArray(rule)) {
    throw new Error("perfect-payload:- framework rule must be an object");
  }

  const configuredSources = Object.keys(rule);
  ...
  return SUPPORTED_REQUEST_SOURCES.filter((source) =>
    Object.prototype.hasOwnProperty.call(rule, source),
  );
}
...
export function validateFrameworkSources({
  ...
  for (const source of configuredSources) {
    ...
    const result = perfectPayload(sourceData, rule[source], options);
